...
let v1 (m: Map<string, string>) = btw (Map.find "byr" m |> int) 1920 2020
let v2 (m: Map<string, string>) = btw (Map.find "iyr" m |> int) 2010 2020
let v3 (m: Map<string, string>)= btw (Map.find "eyr" m |> int) 2020 2030
let v4 (m: Map<string, string>) = 
    let h:string = Map.find "hgt" m 
    match h.[h.Length-2..] with
        | "cm" -> btw (h.[..h.Length-3] |> int) 150 193
        | "in" -> btw (h.[..h.Length-3] |> int) 59 76
        | _ -> false
let v5 (m: Map<string, string>) = 
    let h: string =Map.find "hcl" m 
    let numalpha = ['a'..'f']@['0'..'9']
    let valid_len = Seq.filter (fun x -> Seq.contains x numalpha) h |> Seq.length
    valid_len=6 && h.[0]='#'
let v7 (m: Map<string, string>) = [for c in (Map.find "pid" m) -> List.contains c ['0'..'9']] |> (fun x-> x.Length=6)
let all_ok m = allbutcid m && v1 m && v2 m && v2 m && v4 m && v5 m && v6 m && v7 m
